...
Async subagents for [pi](https://github.com/badlogic/pi-mono), running in tmux or zellij panes. Spawn a sub-agent, keep working in the main session, and get the result steered back when it finishes. Fully non-blocking.

This fork supports tmux and zellij. See [Acknowledgements](#acknowledgements) for the upstream project, which also supports cmux and WezTerm.

## How it works

`subagent()` returns immediately. The sub-agent runs in its own multiplexer pane. tmux creates a right split off the parent pi pane; zellij chooses a tiled split while space permits, then a stacked pane or new tab. A live widget above the input tracks every running sub-agent, and when one finishes, its result is steered into the main session as a notification that triggers a new turn.
...
tmux panes are kept evenly sized: the extension re-applies an `even-horizontal` layout after every spawn and exit (debounced). The layout is the `SUBAGENT_TMUX_LAYOUT` constant in `pi-extension/subagents/mux.ts`. For zellij, minimum usable pane dimensions can be adjusted with `PI_SUBAGENT_ZELLIJ_MIN_COLUMNS` (default `50`) and `PI_SUBAGENT_ZELLIJ_MIN_ROWS` (default `10`).

When both runtimes are visible, set `PI_SUBAGENT_MUX=tmux|zellij` to force a backend.
...
- One supported multiplexer: [tmux](https://github.com/tmux/tmux) or [zellij](https://zellij.dev)
